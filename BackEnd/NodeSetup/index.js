...
app.get('/', async (req, res) => {
  res.status(200).json({ message: "App working right now." })
});
...
const calculateCollectionDateTime = (collectionDate, nextPayoutDate) => {
  const collectionDays = collectionDate === '1 Week' ? 6 : collectionDate === '3 Weeks' ? 20 : 0
...
app.get('/paymentNotification', async (req, res) => {
  ...
  res.status(200).json(pendingPaymentUser)
});

app.get('/inviteNotification', async (req, res) => {
  const committeeMemberData = await getCommitteeMembers()
  const committeeData = await getCommittees()
  const userData = await getUsers()
  ...
  res.status(200).json(pendingCommitteeMembers)
});
